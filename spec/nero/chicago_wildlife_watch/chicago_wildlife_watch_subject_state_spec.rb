...
describe Nero::ChicagoWildlifeWatch::ChicagoWildlifeWatchSubjectState do
  ...
  describe '#retired?' do
    ...
    it "is retired when any classification says it's reported" do
      subject_state.add_vote "RCCN"
      subject_state.add_vote "RCCN"
      expect { subject_state.add_vote "reported" }.to change { subject_state.retired? }.from(false).to(:flagged)
    end
    ...
    it "is retired when seven classifications agree" do
      ...
      subject_state.add_vote "RCCN"
      subject_state.add_vote "RCCN"
      expect { subject_state.add_vote "RCCN" }.to change { subject_state.retired? }.from(false).to(:consensus)
    end
  end
end
