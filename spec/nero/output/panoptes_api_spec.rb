...
describe Nero::Output::PanoptesApi do
  subject(:api) { described_class.new("staging", "key", "secret") }

  before do
    stub_request(:post, "https://panoptes-staging.zooniverse.org/oauth/token")
  ...
  describe '#retire' do
    ...
    it 'retires subjects' do
      request = stub_request(:post, "https://panoptes-staging.zooniverse.org/api/workflows/1/retired_subjects")
    ...
    it 'raises an error if panoptes responds with something other than HTTP 200 status' do
      stub_request(:post, "https://panoptes-staging.zooniverse.org/api/workflows/1/retired_subjects")
